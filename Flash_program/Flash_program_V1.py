...
print(f"Successfully loaded {FIRMWARE_FILE}! Total size: {total_size} bytes ({total_chunks} packets)") 
...
try: 
    ...
    print(f"Sending start command (0x07) with Version : {FIRMWARE_VERSION:02X} to STM32...")
    ...
    for i in range(total_chunks):
        ...
        if ack == STM_ACK_PAUSE:
            ...
            print("    -> Sending WAIT command (0x20) for STM32 to write flash memory...")
            ...
            if ack == STM_ACK_READY:
                print("    -> STM32 is ready (Received 0x49). Resuming transmission loop!\n")
            else:
        ...
        if ack == STM_ACK_READY:
            ...
            print(f"[{i+1}/{total_chunks}] Sending chunk {len(chunk_data)} bytes (0x67)...")
